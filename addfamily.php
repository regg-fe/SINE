...
<body>
	...
	<div class="Btn-menu center container">
		<a class="left" style="margin-left:-60px !important" href="apartments.php?id=<?php echo $_GET['apartamento'] ?>" title="Volver"><i class="fas fa-arrow-left"></i></a>
	</div>
		...
		$(document).ready(function () {
			...
			$("input[name='Genero']").click(function () {
				switch ($("input[name='Genero']:checked").val()) {
					case 'M':
						$("#formulario input[name='Embarazo'][value='S']").prop("checked",false);
						$("#formulario input[name='Embarazo'][value='N']").prop("checked",true);
						...
						break;
					case 'F':
						$("#formulario input[name='Embarazo'][value='N']").prop("checked",false);
						$("#formulario input[name='Embarazo'][value='S']").prop("checked",false);
			...
			$("#add").click(function (ev) {
				...
				var dniExiste = false;
				var jefeExiste = false;
				var tempdni = $("#formulario input[name='DNI']").val();
				if (tempdni == ""){
					tempdni = -1;
				}
				$.post("verifyDNI.php",{
					...
					if ($("#formulario select[name='Posicion']").val() != undefined){
						for (var i = 0 ; i < data.length ; i++){
							if ($("#formulario select[name='Posicion']").val() == data[i]['POSICION'])
								jefeExiste = true;
						}
						if (jefeExiste)
							mensajeError = "Ya hay una persona agregada como jefe de familia</br>";
							//alert("Ya hay una persona agregada como jefe de familia");	//AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
					}
					if (data == 1){
						//alert("Esta cedula ya se encuentra registrada"); //AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
						dniExiste = true;
					}else if ($("#tabla tbody tr").length) {
						for (var i = 0 ; i < data.length ; i++){
							if ($("#formulario input[name='DNI']").val() == data[i]['DNI'])
								dniExiste = true;
						}
						if (dniExiste){
							mensajeError = "Esta cedula ya se encuentra agregada</br>";
							//alert("Esta cedula ya se encuentra agregada");	//AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
						}
			...
			$("#create").click(function (ev) {
				...
				if ($("#tabla tbody tr").length) {
					...
					var bool = false;
					for (var i = 0 ; i < data.length ; i++){
						if (data[i]['POSICION'] == "JEFE")
							bool = true;
					}
					var addToBD = function (data) {
						$.post("addfam.php",{
							data: data,
					...
					if (bool)
						...
						alert("No se ha agregado aún al jefe de familia");	//AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
					}
				}
